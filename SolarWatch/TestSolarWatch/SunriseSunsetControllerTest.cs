...
public class SunriseSunsetControllerTest
{
    ...
    public async Task GetCurrent_ValidInput_ReturnsOkResult()
    {
        ...
        _latLonProviderMock?.Setup(mock => mock.GetCurrent(cityName)).ReturnsAsync(latLonData);
    ...
    [Test]
    public async Task GetCurrent_LatLonProviderThrowsException_ReturnsNotFound()
    {
        // Arrange
        DateTime date = DateTime.Parse("2023-09-10");
    ...
    public async Task GetCurrent_SunriseSunsetProviderThrowsException_ReturnsNotFound()
    {
        ...
        _latLonProviderMock.Setup(mock => mock.GetCurrent(cityName)).ReturnsAsync((string cityName) => latLonData);
        _jsonProcessorMock.Setup(mock => mock.ProcessLat(latLonData)).Returns(47.4979937);
        _jsonProcessorMock.Setup(mock => mock.ProcessLon(latLonData)).Returns(19.0403594);
        _sunriseSunsetProviderMock.Setup(mock => mock.GetCurrent(It.IsAny<string>(), 47.4979937, 19.0403594))
            .Throws<Exception>();
